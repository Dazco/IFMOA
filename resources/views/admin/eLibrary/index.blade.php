                        ...
                        <table class="table table-hover table-striped" id="all_eLibrary">
                            ...
                            @if($eLibrary)
                                @for($i= 1;$i<=count($eLibrary);$i++)
                                    <tr>
                                        <td>{{$i}}</td>
                                        <td><a href="{{action('Admin\AdminELibraryController@download',$eLibrary[$i-1]->id)}}">{{$eLibrary[$i-1]->title}}</a></td>
                                        ...
                                        <td><a href="{{route('admin.eLibrary.edit',$eLibrary[$i-1]->id)}}" class="btn btn-primary">Edit</a></td>
                                        <td>
                                            {!! Form::open(['method'=>'DELETE','action'=>['Admin\AdminELibraryController@destroy',$eLibrary[$i-1]->id],'onsubmit'=>"return confirm('Do you really want to delete this resource?')"]) !!}
