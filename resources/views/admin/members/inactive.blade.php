            ...
            <div class="col-md-12">
                <div class="card striped-tabled-with-hover">
                    <div class="card-header ">
                        <h4 class="card-title">IN-ACTIVE MEMBERS</h4>
                        <p class="card-category">The following are all the in-active members registered on the site</p>
                    </div>
                ...
                <div class="card-body ">
                    @if($grades)
                        @foreach($grades as $grade)
                            <div class="row">
                                <p class="col-6">TOTAL IN-ACTIVE ACTIVE {{$grade->name}}</p>
                                <p class="col-6">{{$grade->users()->where('is_active','0')->where('is_approved','1')->count()}}</p>
                            </div>
                        @endforeach
                    @endif
                </div>
